(function (global, factory) {
    ...
    (global = global || self, factory(global.intlIo = {}));
})(this, (function (exports) {
    var NumberCulture = /*#__PURE__*/function () {
      ...
      _proto.parse = function parse(text) {
        ...
        var clean = '';
        for (var i = 0; i < text.length; i++) if (text[i] == this.decimalSeparator || text[i] >= '0' && text[i] <= '9' || text[i] == '-') clean += text[i];
    ...
    var DateTimeCulture = /*#__PURE__*/function () {
      function DateTimeCulture(cultures, options) {
        this.cultures = cultures;
        this.options = options || {};
      }
      var _proto = DateTimeCulture.prototype;
      _proto.format = function format(date, options, timezone) {
        return this.getFormatter(options, timezone).format(date);
      };
      _proto.getFormatter = function getFormatter(options, timezone) {
        return new Intl.DateTimeFormat(this.cultures, parseOptions(options, timezone || this.options.defaultTimezone));
      };
      ...
      _proto.parse = function parse(text, _temp) {
        ...
        dateParts.alphas.forEach(function (value) {
          for (var i = 0; i < 12; i++) for (var fmt in _this.monthNames[i]) if (value === _this.monthNames[i][fmt].toLowerCase()) {
            result.month = i + 1;
            return;
          }
          unmatchedPart = true;
        ...
        dateParts.numbers.forEach(function (value) {
          if (value > 31) result.year = value;else if (value > 12 && result.date === undefined) result.date = value;else {
        ...
        for (var i = 0; i < Math.min(timeParts.numbers.length, timeComponent.length); i++) result[timeComponent[i]] = timeParts.numbers[i];
      ...
      _proto.load = function load() {
        ...
        var i;
        var monthNames = [];
        for (var m = 0; m < 12; m++) monthNames.push({});
    ...
    function parseOptions(fmt, timezone) {
      ...
      for (var i = 0; i < fmt.length; i++) count[fmt[i]]++;
      var options = {};
      ...
      if (utc > 0) options.timeZone = 'UTC';else if (timezone != null) options.timeZone = timezone;
      return options;
    }
...
//# sourceMappingURL=intl-io.umd.js.map
