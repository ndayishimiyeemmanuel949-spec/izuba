class NumberCulture {
  ...
  parse(text) {
    ...
    let clean = '';
    for (let i = 0; i < text.length; i++) if (text[i] == this.decimalSeparator || text[i] >= '0' && text[i] <= '9' || text[i] == '-') clean += text[i];
...
const weekdayFormats = ['narrow', 'short', 'long'];
const monthFormats = ['narrow', 'short', 'long'];
const defaultOptions = {
  useCurrentDateForDefaults: false,
  loose: false
};
class DateTimeCulture {
  ...
  parse(text, {
    ...
    let {
      date,
      time
    } = splitDateAndTime(strip8206(text));
    ...
    dateParts.alphas.forEach(value => {
      for (let i = 0; i < 12; i++) for (let fmt in this.monthNames[i]) if (value === this.monthNames[i][fmt].toLowerCase()) {
        result.month = i + 1;
        return;
      }
      unmatchedPart = true;
    ...
    for (let i = 0; i < Math.min(timeParts.numbers.length, timeComponent.length); i++) result[timeComponent[i]] = timeParts.numbers[i];
  ...
  load() {
    ...
    monthFormats.forEach(monthFormat => {
      let dateFormat = new Intl.DateTimeFormat(this.cultures, {
        month: monthFormat
      });
      for (i = 0; i < 12; i++) {
    ...
    let localeDateFmt = localeDate.replace(2077, 'year').replace(11, 'month').replace(22, 'date');
...
//# sourceMappingURL=intl-io.modern.mjs.map
