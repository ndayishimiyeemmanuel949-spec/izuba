...
export class ColumnGraph extends ColumnBarGraphBase {
    ...
    renderGraph(context, instance) {
        ...
        return data.data.map((p, i) => {
            ...
            return (_jsx("rect", { className: this.CSS.element(this.baseClass, "column", state), onClick: (e) => {
                    this.handleClick(e, instance, p, i);
                }, x: Math.min(x1, x2), y: Math.min(y1, y2), width: Math.abs(x2 - x1), height: Math.abs(y2 - y1), style: data.style, onMouseMove: mmove, onMouseLeave: mleave, rx: data.borderRadius }, i));
        });
    }
}
