...
export class TimeAxis extends Axis {
    ...
    explore(context, instance) {
        ...
        instance.calculator.reset(min, max, this.snapToTicks, this.tickDivisions, Math.max(1, this.minTickDistance), Math.max(1, this.minLabelDistance), normalized, inverted, this.minTickUnit, lowerDeadZone, upperDeadZone, this.decode, this.useLabelDistanceFormatOverrides
            ? this.minLabelDistanceFormatOverride
            : {}, this.format);
    }
    render(context, instance, key) {
        ...
        return (_jsx("g", { className: data.classNames, style: data.style, children: this.renderTicksAndLabels(context, instance, formatter, minLabelDistance) }, key));
    }
}
...
class TimeScale {
    ...
    shouldUpdate;
    reset(min, max, snapToTicks, tickDivisions, minTickDistance, minLabelDistance, normalized, inverted, minTickUnit, lowerDeadZone, upperDeadZone, decode, minLabelDistanceFormatOverride, format) {
        this.dateCache = {};
    ...
    getFormat(unit, scale) {
        switch (unit) {
            ...
            case "month":
                if (new Date(scale.min).getFullYear() != new Date(scale.max).getFullYear())
                    return "yearOrMonth";
                return "datetime;yyyy MMM";
            case "week":
                return "datetime;MMMdd";
            case "day":
                if (new Date(scale.min).getFullYear() !=
    ...
    trackValue(v, offset = 0, constrain = false) {
        let value = (v - this.origin) / this.scale.factor -
            offset +
            this.scale.min -
    ...
    calculateTicks() {
        ...
        for (let unit in milliSeconds) {
            ...
            for (let i = 0; i < divisions.length; i++) {
                ...
                for (let d = 0; d < divs.length; d++) {
                    ...
                    let minLabelDistance = this.minLabelDistanceFormatOverride[format] ??
        ...
        if (lowerTickUnit != null && this.scale) {
            ...
            for (let i = 0; i < divisions.length; i++) {
                ...
                for (let d = 0; d < divs.length; d++) {
                    let tickSize = divs[d] * unitSize;
                    if (tickSize * Math.abs(this.scale.factor) >= this.minTickDistance &&
    ...
    getTicks(tickSizes) {
        return tickSizes.map(({ size, measure }) => {
            ...
            if (measure == "year") {
                ...
                let date = Math.ceil((this.scale.min -
                    this.scale.minPadding -
                    minOffset -
                    mondayOffset) /
                    size) *
