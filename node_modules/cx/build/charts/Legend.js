...
export class Legend extends HtmlElement {
    ...
    renderChildren(context, instance) {
        const CSS = this.CSS;
        let entries = instance.legends[this.name] && instance.legends[this.name].entries, list;
        ...
        if (isNonEmptyArray(entries)) {
            list = entries.map((e, i) => withHoverSync(i, e.hoverSync, e.hoverChannel, e.hoverId, ({ onMouseMove, onMouseLeave, hover }) => (_jsxs("div", { className: CSS.expand(CSS.element(this.baseClass, "entry", {
                    "color-root": true,
                    hover,
                    disabled: e.disabled,
                    selected: e.selected,
                }), entryClass), style: entryStyle, onClick: e.onClick, onMouseMove: onMouseMove, onMouseLeave: onMouseLeave, children: [this.renderShape(e, shape), _jsx("div", { className: entryTextClass, children: e.displayText || e.name }), this.showValues && (_jsx("div", { className: valueClasses, style: valueStyle, children: valueFormatter(e.value) }))] }, i))));
        }
    ...
    renderShape(entry, legendEntriesShape) {
        ...
        if (hasVisibleValue(style.stroke) && !style.fill)
            style.fill = style.stroke;
        if (hasVisibleValue(style.fill) && !style.stroke)
            style.stroke = style.fill;
        return (_jsx("svg", { className: this.CSS.element(this.baseClass, "svg"), style: {
