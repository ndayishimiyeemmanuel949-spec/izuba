...
export class Swimlanes extends BoundedObject {
    ...
    render(context, instance, key) {
        ...
        while (at - d.size / 2 < max) {
            ...
            if (this.vertical) {
                rects.push(_jsx("rect", { y: bounds.t, x: Math.min(c1, c2), height: bounds.b - bounds.t, width: Math.abs(c1 - c2), className: rectClass, style: d.laneStyle }, index++));
            }
            else {
                rects.push(_jsx("rect", { x: bounds.l, y: Math.min(c1, c2), width: bounds.r - bounds.l, height: Math.abs(c1 - c2), className: rectClass, style: d.laneStyle }, index++));
            }
            at += d.step;
        }
        return (_jsx("g", { className: d.classNames, children: rects }, key));
    }
}
