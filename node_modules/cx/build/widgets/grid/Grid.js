...
export class Grid extends ContainerBase {
    ...
    renderGroupHeader(context, instance, g, level, group, i, store, fixedColumns) {
        ...
        if (g.caption) {
            ...
            return (_jsx("tbody", { className: CSS.element(baseClass, "group-caption", ["level-" + level, resetRowNumbers]), "data-group-key": group.$key, "data-group-element": `group-caption-${level}`, children: _jsx("tr", { children: _jsx("td", { colSpan: 1000, children: caption }) }) }, `g-${level}-${group.$key}`));
        }
            ...
            if (lines.length == 0)
                return null;
            return (_jsx("tbody", { className: CSS.element(baseClass, "group-caption", ["level-" + level, resetRowNumbers]), "data-group-key": group.$key, "data-group-element": `group-caption-${level}`, children: lines }, "c" + group.$key));
        }
    }
