...
import { Container, StyledContainerBase } from "../ui/Container";
 ...
 */
export class List extends StyledContainerBase {
    constructor(config) {
        super(config);
    }
    init() {
    ...
    render(context, instance, key) {
        ...
        return (_jsx(ListComponent, { instance: instance, items: items, selectable: (!this.selection.isDummy || !!this.onItemClick) }, key));
    }
    groupBy(grouping) {
        if (!isArray(grouping)) {
...
class ListComponent extends VDOM.Component {
        ...
        let children = items.length > 0 &&
            items.map((x, i) => {
                ...
                if (x.type == "data") {
                    ...
                    return (_jsx("li", { className: CSS.expand(className, data.classNames), style: itemStyle, "data-record-index": ind, onMouseDown: this.handleItemMouseDown, onClick: onClick, onDoubleClick: onDblClick, children: x.content }, x.key));
                }
                else {
                    return (_jsx("li", { className: CSS.element(baseClass, x.type), children: x.content }, x.key));
                }
            });
        if (!children && data.emptyText) {
            children = _jsx("li", { className: CSS.element(baseClass, "empty-text"), children: data.emptyText });
        }
        return (_jsx("ul", { ref: (el) => {
                this.el = el;
            }, className: CSS.expand(data.classNames, CSS.state({ focused: this.state.focused })), style: data.style, tabIndex: widget.focusable && selectable && items.length > 0 ? data.tabIndex || 0 : undefined, onMouseDown: preventFocusOnTouch, onKeyDown: this.handleKeyDown.bind(this), onMouseLeave: this.handleMouseLeave.bind(this), onFocus: this.onFocus.bind(this), onBlur: this.onBlur.bind(this), children: children }));
    }
    componentDidUpdate() {
    ...
    scrollElementIntoView() {
        ...
        if (firstSelectedRow != this.selectedEl) {
            if (firstSelectedRow)
                scrollElementIntoView(firstSelectedRow, true, false, 0, this.el);
    ...
    moveCursor(index, { focused, hover, scrollIntoView, select, selectRange, selectOptions, } = {}) {
        ...
        batchUpdates(() => {
            if (select || widget.selectMode) {
                let start = selectRange && this.state.selectionStart !== undefined && this.state.selectionStart >= 0
    ...
    showCursor(force, newItems) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (isDataItem(item)) {
                index++;
                if (!isItemDisabled(item) && firstValid == -1)
                    firstValid = index;
    ...
    handleKeyDown(e) {
        ...
        switch (e.keyCode) {
            case KeyCode.tab:
            case KeyCode.enter:
                if (!widget.selectOnTab && e.keyCode == KeyCode.tab)
                    break;
