...
export class DropdownBase extends OverlayBase {
    ...
    overlayDidMount(instance, component) {
        ...
        if (instance.parentPositionChangeEvent)
            component.offParentPositionChange =
                instance.parentPositionChangeEvent.subscribe(component.updateDropdownPosition);
    }
    ...
    updateDropdownPosition(instance, component) {
        ...
        let explode = this.pad && typeof this.elementExplode === "number"
            ? this.elementExplode
            : 0;
        if (explode) {
    ...
    getArrowAdjust(component) {
        if (!this.alignArrow || !this.arrow)
            return 0;
        if (component.cachedArrowOffset != null)
            return component.cachedArrowOffset;
        if (!component.el)
            return 0;
    ...
    applyPositioningPlacementStyles(style, placement, contentSize, parentBounds, el, noAuto, arrowAdjust = 0) {
        switch (this.positioning) {
            case "absolute":
                this.applyAbsolutePositioningPlacementStyles(style, placement, contentSize, parentBounds, el, noAuto, arrowAdjust);
                break;
            case "auto":
                if (isTouchDevice())
                    this.applyAbsolutePositioningPlacementStyles(style, placement, contentSize, parentBounds, el, noAuto, arrowAdjust);
        ...
        for (var i = 0; i < placementOrder.length; i++) {
            ...
            switch (primary) {
                case "down":
                    score[p] +=
                        3 *
                            Math.min(1, (viewport.bottom - target.bottom - this.offset) /
                                contentSize.height);
                    break;
                case "up":
                    score[p] +=
                        3 *
                            Math.min(1, (target.top - viewport.top - this.offset) / contentSize.height);
                    break;
                case "right":
                    score[p] +=
