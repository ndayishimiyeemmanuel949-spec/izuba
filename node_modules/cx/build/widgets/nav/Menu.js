...
export class Menu extends HtmlElement {
    ...
    render(context, instance, key) {
        return (_jsx(MenuComponent, { instance: instance, children: this.renderChildren(context, instance) }, key));
    }
    add(item) {
...
export class MenuInstance extends HtmlElementInstance {
}
class MenuComponent extends VDOM.Component {
    ...
    render() {
        ...
        return (_jsx("ul", { ref: this.ref, className: CSS.expand(data.classNames, CSS.state({
                pack: this.state.nonOverflownItemCount < children.length - 1,
            })), style: data.style, onBlur: FocusManager.nudge, onKeyDown: this.onKeyDown.bind(this), children: children.map((content, index) => {
                let key = content && typeof content == "object" && content.key ? content.key : index;
                if (content && content.spacer) {
                    return widget.horizontal && _jsx("li", { className: CSS.element(baseClass, "spacer") }, key);
                }
                return (_jsx(MenuItemComponent, { cursor: key === this.state.cursor, hidden: index < parentNonOverflownItemCount ||
                        (index >= this.state.nonOverflownItemCount && index + 1 != children.length), instance: instance, itemInfo: this.itemInfo, itemKey: key, itemIndex: index, moveCursor: this.moveCursor.bind(this), children: content }, key));
            }) }));
    }
    moveCursor(itemKey) {
    ...
    measureOverflow() {
        ...
        let widths = children.map((c) => {
            if (!(c instanceof HTMLElement))
                return 0;
            if (c.classList.contains(spacerClass))
                return 0;
            let w = c.offsetWidth;
        ...
        for (let i = 0; i < widths.length - 1; i++) {
            ...
            if (widths[i] + fitItemsWidth > clientWidth - overflowReduction)
                break;
...
class MenuItemComponent extends VDOM.Component {
    ...
    render() {
        ...
        return (_jsx("li", { ref: (c) => {
            ...
            }, className: CSS.element(baseClass, "item", mods), onFocus: this.onFocus.bind(this), onMouseDown: this.onMouseDown.bind(this), onKeyDown: this.onKeyDown.bind(this), children: this.props.children }));
    }
    onFocus(e) {
        FocusManager.nudge();
