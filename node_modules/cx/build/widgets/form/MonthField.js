...
export class MonthFieldInstance extends FieldInstance {
    lastDropdown;
    dropdownOpen;
    selectedIndex;
    component;
...
export class MonthField extends Field {
    ...
    init() {
        if (!this.culture)
            this.culture = Culture.getDateTimeCulture();
    ...
    renderInput(context, instance, key) {
        return (_jsx(MonthInput, { data: instance.data, instance: instance, monthPicker: {
                mod: "dropdown",
...
class MonthInput extends VDOM.Component {
    ...
    render() {
        ...
        if (!data.readOnly && !data.disabled) {
            if (widget.showClear &&
                ...
                insideButton = (_jsx("div", { className: CSS.element(baseClass, "clear"), onMouseDown: (e) => {
                        ...
                        this.onClearClick(e);
                    }, children: _jsx(ClearIcon, { className: CSS.element(baseClass, "icon") }) }));
            else
                insideButton = (_jsx("div", { className: CSS.element(baseClass, "right-icon"), children: _jsx(DropdownIcon, { className: CSS.element(baseClass, "icon") }) }));
        }
        if (iconVDOM) {
            icon = (_jsx("div", { className: CSS.element(baseClass, "left-icon"), children: iconVDOM }));
        }
        var dropdown = false;
        if (this.state.dropdownOpen)
            dropdown = (_jsx(Cx, { widget: this.getDropdown(), parentInstance: instance, options: { name: "monthfield-dropdown" }, subscribe: true }));
        ...
        return (_jsxs("div", { className: CSS.expand(data.classNames, CSS.state({
            ...
            })), style: data.style, onMouseDown: this.onMouseDown.bind(this), onTouchStart: stopPropagation, onClick: stopPropagation, children: [_jsx("input", { id: data.id, ref: (el) => {
                        this.input = el;
                    }, type: "text", className: CSS.expand(CSS.element(baseClass, "input"), data.inputClass), style: data.inputStyle, defaultValue: data.formatted, disabled: data.disabled, readOnly: data.readOnly, tabIndex: data.tabIndex, placeholder: data.placeholder, onInput: (e) => this.onChange(e.target.value, "input"), onChange: (e) => this.onChange(e.target.value, "change"), onKeyDown: (e) => this.onKeyDown(e), onBlur: (e) => {
                        this.onBlur(e);
                    }, onFocus: (e) => {
                        this.onFocus(e);
                    }, onMouseMove: (e) => tooltipMouseMove(e, ...getFieldTooltip(this.props.instance)), onMouseLeave: (e) => tooltipMouseLeave(e, ...getFieldTooltip(this.props.instance)) }), icon, insideButton, dropdown, label, help] }));
    }
    onMouseDown(e) {
        e.stopPropagation();
        if (this.state.dropdownOpen)
    ...
    UNSAFE_componentWillReceiveProps(props) {
        ...
        if (data.formatted != this.input.value &&
    ...
    onChange(inputValue, eventType) {
        ...
        if ((date1 != null && isNaN(date1.getTime())) ||
