...
export class MonthPickerInstance extends FieldInstance {
...
export class MonthPicker extends Field {
    ...
    prepareData(context, instance) {
        ...
        if (this.range) {
            if (data.from)
                data.from = monthStart(parseDateInvariant(data.from));
            if (data.to) {
        ...
        if (this.onCreateIsMonthDateSelectable) {
            instance.isMonthDateSelectable = instance.invoke("onCreateIsMonthDateSelectable", data.validationParams, instance);
        }
    ...
    renderInput(context, instance, key) {
        return (_jsx(MonthPickerComponent, { instance: instance, onBlur: this.onBlur, onFocusOut: this.onFocusOut, onKeyDown: this.onKeyDown, autoFocus: this.autoFocus }, key));
    }
    handleSelect(e, instance, date1, date2) {
        ...
        if (data.disabled)
            return;
        if (isMonthDateSelectable && !isMonthDateSelectable(date1))
            return;
        if (!dateSelectableCheck(date1, data))
            return;
        ...
        if (this.range) {
            ...
            instance.set("value", encode(date1));
        if (this.onSelect)
...
const dateSelectableCheck = (date, data) => {
    if (data.maxValue &&
        !upperBoundCheck(date, data.maxValue, data.maxExclusive))
        return false;
    if (data.minValue &&
        !lowerBoundCheck(date, data.minValue, data.minExclusive))
        return false;
    return true;
};
...
export class MonthPickerComponent extends VDOM.Component {
        ...
        switch (e.keyCode) {
            ...
            case KeyCode.left:
                if (column == "Y")
                    ...
                    this.moveCursor(e, { cursorMonth: cursorQuarter * 4, column: "M" }, { select });
                else if (column == "M" && (cursorMonth - 1) % 3 == 0)
                    this.moveCursor(e, { column: "Y" });
                else
                    this.moveCursor(e, { cursorMonth: cursorMonth - 1 }, { select });
                break;
            case KeyCode.right:
                if (column == "Y")
            ...
            case KeyCode.up:
                ...
                if (column == "Y")
                    this.moveCursor(e, { cursorYear: cursorYear - 1 }, { ensureVisible: true, select });
                else if (column == "Q")
                    this.moveCursor(e, {
                    ...
                    if (cursorMonth > 3)
                        this.moveCursor(e, { cursorMonth: cursorMonth - 3 }, { ensureVisible: true, select });
                    else
                        this.moveCursor(e, { cursorMonth: cursorMonth + 9, cursorYear: cursorYear - 1 }, { ensureVisible: true, select });
                break;
            case KeyCode.down:
                e.preventDefault(); // prevent scroll
                if (column == "Y")
                ...
                break;
            case KeyCode.pageUp:
                this.moveCursor(e, { cursorYear: this.state.cursorYear - 1 }, { ensureVisible: true, select });
                break;
    ...
    handleFocusOut(e) {
        ...
        if (widget.onFocusOut)
            instance.invoke("onFocusOut", e, instance);
    }
    ...
    render() {
        ...
        for (let y = start; y <= end; y++) {
            ...
            for (let q = 0; q < 4; q++) {
                let row = [];
                if (q == 0) {
                    row.push(_jsx("th", { rowSpan: 4, "data-point": `Y-${y}`, className: CSS.expand(CSS.element(baseClass, "year", {
                            cursor: showCursor &&
                                this.state.column == "Y" &&
                                y == this.state.cursorYear,
                        }), CSS.state({ unselectable: unselectableYear })), onMouseEnter: unselectableYear ? undefined : this.handleMouseEnter, onMouseDown: unselectableYear ? undefined : this.handleMouseDown, onMouseUp: unselectableYear ? undefined : this.handleMouseUp, children: y }, "year"));
                }
                for (let i = 0; i < 3; i++) {
                    let m = q * 3 + i + 1;
                    ...
                    row.push(_jsx("td", { className: CSS.state({
                            cursor: showCursor &&
                                this.state.column == "M" &&
                                y == this.state.cursorYear &&
                ...
                if (!hideQuarters) {
                    ...
                    row.push(_jsx("th", { className: CSS.state({
                            cursor: showCursor &&
                                this.state.column == "Q" &&
                                y == this.state.cursorYear &&
                                q == this.state.cursorQuarter,
                            unselectable: unselectableQuarter,
                        }), "data-point": `Y-${y}-Q-${q}`, onMouseEnter: unselectableQuarter ? undefined : this.handleMouseEnter, onMouseDown: unselectableQuarter ? undefined : this.handleMouseDown, onMouseUp: unselectableQuarter ? undefined : this.handleMouseUp, children: `Q${q + 1}` }, `q${q}`));
                }
                rows.push(row);
            }
            years.push(rows);
        ...
        return (_jsxs("div", { ref: (el) => {
                this.dom.el = el;
            }, className: data.classNames, style: data.style, tabIndex: data.disabled ? null : data.tabIndex || 0, onKeyDown: this.handleKeyPress, onMouseDown: stopPropagation, onMouseMove: (e) => tooltipMouseMove(e, ...getFieldTooltip(this.props.instance)), onMouseLeave: this.handleMouseLeave.bind(this), onFocus: (e) => this.handleFocus(e), onBlur: this.handleBlur.bind(this), onScroll: this.onScroll.bind(this), children: [this.state.yearHeight && (_jsx("div", { style: {
                        height: `${(start - startYear) * this.state.yearHeight}px`,
                    } })), _jsx("table", { ref: (el) => {
                        this.dom.table = el;
                    }, children: years.map((rows, y) => (_jsx("tbody", { children: rows.map((cells, i) => (_jsx("tr", { children: cells }, i))) }, start + y))) }), this.state.yearHeight && (_jsx("div", { style: {
    ...
    onScroll() {
        ...
        let start = Math.max(startYear, startYear +
