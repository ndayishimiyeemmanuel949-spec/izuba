...
export class LookupField extends Field {
    ...
    init() {
        ...
        if (this.infinite && this.onQuery && !this.onQueryPage)
            this.onQueryPage = this.onQuery;
    ...
    prepareData(context, instance) {
        ...
        if (this.validateOptionExists &&
            isArray(data.options) &&
            !this.isEmpty(data)) {
            let invalid = this.multiple
                ? isArray(data.values) && data.records.length < data.values.length
                : !data.options.some(($option) => areKeysEqual(getOptionKey(this.keyBindings, { $option }), data.selectedKeys[0]));
            if (invalid)
                data.error = this.invalidOptionText;
        }
        super.prepareData(context, instance);
    }
    renderInput(context, instance, key) {
        return (_jsx(LookupComponent, { multiple: this.multiple, instance: instance, itemConfig: this.itemConfig, bindings: this.bindings, baseClass: this.baseClass, label: this.labelPlacement &&
                getContent(this.renderLabel(context, instance, "label")), help: this.helpPlacement &&
                getContent(this.renderHelp(context, instance, "help")), forceUpdate: context.forceUpdate, icon: this.renderIcon(context, instance, "icon") }, key));
    }
    filterOptions(instance, options, query) {
        if (!query)
            return options;
        let textPredicate = getSearchQueryPredicate(query);
        return options.filter((o) => isString(o[this.optionTextField]) &&
    ...
    formatValue(context, instance) {
        ...
        if (isArray(records)) {
            let valueTextFormatter = typeof this.onGetRecordDisplayText === "function"
                ...
                : (record) => record[this.valueTextField] ||
                    record[this.valueIdField];
...
class LookupComponent extends VDOM.Component {
                ...
                if (this.dom.dropdown && this.dom.list) {
                    return {
                        height: this.dom.dropdown.offsetHeight -
                            ...
                            (this.dom.list.firstElementChild?.offsetHeight ||
                                0),
                    };
                }
            },
        ...
        let searchVisible = !widget.hideSearchField &&
            (!isArray(data.visibleOptions) ||
                (widget.minOptionsForSearchField &&
                    data.visibleOptions.length >= widget.minOptionsForSearchField));
    ...
    render() {
        ...
        if (!data.readOnly) {
            if (widget.showClear &&
                !data.disabled &&
                !data.empty &&
                (widget.alwaysShowClear ||
                    (!data.required && !this.props.multiple) ||
                    multipleEntries)) {
                insideButton = (_jsx("div", { onMouseDown: preventDefault, onClick: (e) => !this.props.multiple
                        ? this.onClearClick(e)
                        : this.onClearMultipleClick(e), className: CSS.element(baseClass, "clear"), children: _jsx(ClearIcon, { className: CSS.element(baseClass, "icon") }) }, "ib"));
            }
            else {
        ...
        if (this.props.multiple) {
            ...
            if (isNonEmptyArray(data.records)) {
                ...
                text = (_jsx("div", { className: CSS.element(baseClass, "tags"), children: data.records.map((v, i) => (_jsxs("div", { className: CSS.element(baseClass, "tag", {
                            readonly: readOnly,
            ...
            text = (_jsx("span", { className: CSS.element(baseClass, "text"), children: content }));
        }
        let states = {
    ...
    select(e, itemsData, reset) {
        ...
        if (widget.multiple) {
            ...
            // deselect
            if (singleSelect &&
                selectedKeys.find((k) => areKeysEqual(optionKey, k))) {
        ...
        if (e.keyCode == KeyCode.enter &&
            widget.submitOnDropdownEnterKey) {
    ...
    query(q) {
        ...
        if (widget.infinite ? widget.onQueryPage : widget.onQuery) {
            ...
            this.queryTimeoutId = setTimeout(() => {
                ...
                if (!result) {
                    if (widget.infinite) {
                        let params = { query, page: 1, pageSize };
