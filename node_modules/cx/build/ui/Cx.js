...
export class Cx extends VDOM.Component {
    ...
    UNSAFE_componentWillReceiveProps(props) {
        let newStore = props.instance
            ? props.instance.store
            : props.store
                ? props.store
                : props.parentInstance.store;
    ...
    getInstance() {
        ...
        if (this.instance && this.instance.widget === this.widget) {
            if (this.instance.parentStore != this.store)
                this.instance.setParentStore(this.store);
...
class CxContext extends VDOM.Component {
    ...
    UNSAFE_componentWillReceiveProps(props) {
        ...
        let count = 0, visible, context, forceContinue;
        ...
        if (this.props.cultureInfo)
            pushCulture(this.props.cultureInfo);
        try {
            do {
                count++;
                forceContinue = false;
            ...
            if (this.props.cultureInfo)
                popCulture(this.props.cultureInfo);
        }
    ...
    componentDidUpdate() {
        ...
        if (process.env.NODE_ENV !== "production") {
            ...
            Timing.log(vdomRenderFlag, this.renderCount, "cx", (beforeVDOMRender - start + afterCleanup - afterVDOMRender).toFixed(2) + "ms", "vdom", (afterVDOMRender - beforeVDOMRender).toFixed(2) + "ms");
            Timing.log(appLoopFlag, this.renderCount, this.renderingContext.options.name || "main", "total", (afterCleanup - start).toFixed(1) + "ms", "explore", (afterExplore - start).toFixed(1) + "ms", "prepare", (afterPrepare - afterExplore).toFixed(1), "render", (afterRender - afterPrepare).toFixed(1), "vdom", (afterVDOMRender - beforeVDOMRender).toFixed(1), "cleanup", (afterCleanup - afterVDOMRender).toFixed(1));
        }
    }
    componentWillUnmount() {
        let { instance } = this.props;
