 ...
 */
let subscribers = new SubscriberList(), timerInterval = 300, timerId = null;
...
export function unfocusElement(target = null, unfocusParentOverlay = true) {
    ...
    if (unfocusParentOverlay) {
        ...
        if (focusableOverlayContainer)
            target = focusableOverlayContainer;
    }
    //find the closest focusable parent of the target element and focus it instead
    let focusableParent = closestParent(target, (el) => isFocusable(el) && (!unfocusParentOverlay || !!el.dataset?.focusableOverlayContainer));
