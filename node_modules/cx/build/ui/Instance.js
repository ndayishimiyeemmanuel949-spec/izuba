...
export class Instance {
    ...
    init(context) {
        ...
        if (this.widget.onInit)
            this.widget.onInit(context, this);
    ...
    explore(context) {
        ...
        if (this.widget.isContent) {
            ...
            if (this.contentPlaceholder)
                ...
                if (!list)
                    list = context.contentList[this.widget.putInto] = [];
                list.push(this);
            }
        }
    ...
    set(prop, value, options = {}) {
        ...
        if (setter) {
            if (options.immediate && isFunction(setter.reset))
                setter.reset(value);
            else
                setter(value);
            return true;
        ...
        if (p && typeof p == "object") {
            if (p.debounce) {
                this.definePropertySetter(prop, validatedDebounce((value) => this.doSet(prop, value), () => this.dataSelector(this.store)[prop], p.debounce));
    ...
    nestedDataSet(key, value, dataConfig, useParentStore) {
        ...
        if (!config.set)
            throw new Error(`Cannot change nested data value for ${key} as it's read-only. Either define it as a binding or define a set function.`);
        if (isString(config.set))
            this.getControllerMethod(config.set)(value, this);
        else if (isFunction(config.set))
            config.set(value, this);
        else
            throw new Error(`Cannot change nested data value for ${key} the defined setter is neither a function nor a controller method.`);
        return true;
    }
    replaceState(state) {
        this.cached.state = this.state;
