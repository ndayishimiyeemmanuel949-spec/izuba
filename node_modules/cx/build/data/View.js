...
export class View {
    ...
    delete(path) {
        ...
        if (isArray(path))
            return path.map((arg) => this.delete(arg)).some(Boolean);
        let binding = Binding.get(path);
    ...
    mutate(path, updateFn, ...args) {
        throw new Error("Mutate requires Immer. Please install 'immer' and 'cx-immer' packages and enable store mutation by calling enableImmerMutate().");
    }
    batch(callback) {
