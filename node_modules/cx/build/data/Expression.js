 ...
 */
let helpers = {}, helperNames = [], helperValues = [], expFatArrows = null;
...
export function expression(str) {
    ...
    if (r)
        return r;
    let quote = false;
    let termStart = -1, curlyBrackets = 0, percentExpression;
    let fb = ["return ("];
    let args = {};
    let formats = [];
    let subExprCount = 0;
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        switch (c) {
            case "{":
                ...
                else if (termStart < 0 && (quote || str[i - 1] != "{"))
                    fb.push(c);
                break;
            case "}":
                if (termStart >= 0) {
    ...
    try {
        let compute = new Function("fmt", ...formats.map((f, i) => "fmt" + i), ...helperNames, ...keys, body).bind(Format, Format.value, ...formats, ...helperValues);
