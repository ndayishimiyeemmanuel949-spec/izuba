...
export function computable(...selectorsAndCompute) {
    ...
    for (let i = 0; i + 1 < selectorsAndCompute.length; i++) {
        ...
        if (isString(a) || isAccessorChain(a))
            inputs.push(Binding.get(a.toString()).value);
        else if (a.memoize)
    ...
    function memoize(warmupData) {
        let lastValue, lastArgs = warmupData && inputs.map((s) => s(warmupData));
    ...
    let selector = (data) => compute.apply(null, inputs.map((s) => s(data)));
